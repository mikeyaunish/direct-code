Red [
	...
	Comment: "Extracted from: <root-path>%experiments/modify-source/modify-source.red"
	Date: 19-Mar-2022
	Time: 20:41:08
]
    ...
    dc-modify-source: context [
        ...
        find-insert-position: function [
            ...
            keyword-order: fix-datatype reverse copy [
                ...
                top middle bottom 
                bold italic underline wrap no-wrap no-border 
                font-name font-size font-color font 
                hint image draw 
                data select default extra rate with 
                react url! 
                all-over tri-state 
                actors on-click on-down on-up on-create on-mid-down on-mid-up on-alt-down on-alt-up 
        ...
        set 'modify-source function [{Change source code supplied. Defaults to creating a new entry if it doesn't exist.V19-Feb-2022} 
            source-code 
            obj-name [string!] 
            ...
            obj-info: get-obj-info source-code obj-name target 
            either obj-info/1 [
                ...
                either (target = word-at-block) [
                    ...
                    insert skip source-code insert-position rejoin [pre-value mold new-value post-value]
                ]
            ] 
