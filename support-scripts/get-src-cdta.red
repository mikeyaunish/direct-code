Red [
	...
	Comment: "Extracted from: <root-path>%experiments/get-src-cdta/get-src-cdta.red"
	Date: 19-Mar-2022
	Time: 20:41:08
]
    ...
    dc-get-src-cdta: context [
        trim-non-char: function [str [string!]] [
            trim/with str trim/with copy str {abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ -1234567890:.}
        ] 
        set 'get-src-cdta func [{get source code in 'Canonical Data' (cdta) format } 
            source
        ] [
            ...
            add-chunk: func [event-blk [block!]] [
                ...
                insert (skip event-blk 2) reduce ['object current-object-name] 
                return event-blk
            ] 
            dc-lexer: function [
                event [word!] 
                ...
                either open-block [
                    ...
                    if all [(event = 'close) (chunk-string = "]")] [
                        block-count: block-count - 1 
                        if (block-count = 0) [
                            append open-block token 
                            block-data: copy/part skip full-input (open-block/2/x - 1) (open-block/3/x - open-block/2/x + 1) 
                            chunk-index: chunk-index + 1 
                            append/only res-chunks cls-blk: reduce ['index chunk-index 'object current-object-name 'input block-data 'type block! 'token (to-pair reduce [open-block/2/x open-block/3/x]) 'line (to-pair reduce [open-block/1 line])] 
                            open-block: false 
                            open-chunk-string: FALSE
                        ]
                    ]
