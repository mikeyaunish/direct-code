Red [
    ...
    evo-window-cleanup: function [
        ...
        if fnd: find active-evo-windows window-name [
            remove fnd
        ] 
    ...
    edit-vid-object: function [
        ...
        --evo-window active-evo-windows dc-evo-layout-template-data dc-evo-layout-template-file 
        dc-evo-size
    ] [
        if refresh [
        ] 
        uwin-string: unique-window-name: rejoin ["--evo-window-" obj-name] 
        either find active-evo-windows unique-window-name [
            target-object-name: get to-path reduce [to-word rejoin ["--evo-window-" obj-name] 'extra 'target-object-name] 
            set to-path reduce [to-word unique-window-name 'color] green 
            wait 0:00:00.2 
            set to-path reduce [to-word unique-window-name 'color] white 
            set-focus get to-word rejoin ["name-field" target-object-name] 
            return none
        ] [
        ] 
        target-object-name~: copy obj-name 
        evo-target-object-name: copy target-object-name~ 
        source-code-widget-name: copy src-code-widget-name 
        evo-layout-template: copy dc-evo-layout-template-data 
        uid: get-uid 
        replace/all evo-layout-template "~" rejoin ["~" uid] 
        replace/all evo-layout-template "vid-code-test/text" rejoin [source-code-widget-name "/text"] 
        replace evo-layout-template "awidget" obj-name 
        if refresh [
            replace/all evo-layout-template "refresh-results-gui" refresh-code
        ] 
        replace/all evo-layout-template "refresh-gui" "" 
        replace/all evo-layout-template "evo-target-object-name" (mold obj-name) 
        replace/all evo-layout-template "--evo-window" unique-window-name 
        replace evo-layout-template {title "VID Object Editor"} rejoin [{title "VID Object Editor - [} obj-name {]"}] 
        evo-layout: load evo-layout-template 
        ...
        set unique-window-name layout evo-layout 
        ...
        win-offset: either active-evo-windows <> [] [
            last-win: last active-evo-windows 
        ...
        evo-window-offset: compose [offset: (win-offset)] 
        append active-evo-windows uwin-string 
        set to-path reduce [unique-window-name 'menu] [
            "Object" [
            ...
            "Position" [
                "Insert 'return' before object" insert-return-before 
                "Remove 'return' before object" remove-return-before 
                "" 
                "Move to Specific Position" move-specific 
                "Move to          START   |<----" move-to-beginning 
                "Move Object  Back 4  <<<<" move-back-4 
                "Move Object  Back 3  <<<" move-back-3 
                "Move Object  Back 2  <<" move-back-2 
                "Move Object  Back 1  <" move-back-1 
                "Move Object Ahead 1  >" move-forward-1 
        ...
        requester-window-escape/options compose/deep [
            ...
            evo-window-cleanup (to-string unique-window-name) (uid)
        ] obj-name evo-window-offset
    ] 
    tilde: to-char 126 
    vid-cdta-to-view-fields: function [
        vid-cdta [block! none!] 
        /id identifier
    ] [
        if (not vid-cdta) [return none] 
        identifier: to-string identifier 
        replace identifier (to-string tilde) "" 
        foreach record vid-cdta [
            record-id: fix-datatype (reduce [record/1 record/2]) 
            record-value: record/3 
            line-status: copy "" 
            if not (record-value = none) [
                field-name: either block? record/1 [
                    rejoin [to-string record/1/1 "-" to-string record/1/2 "-field" tilde identifier]
                ] [
                    rejoin [to-string record/1 "-field" tilde identifier]
                ] 
                switch/default (to-string record/2) [
                    "logic" [
                        set (to-path reduce [to-word field-name 'data]) record-value
                    ] 
                    "block" [
                        set (to-path reduce [to-word field-name 'text]) (mold record-value)
                    ] 
                    "file" [
                        set (to-path reduce [to-word field-name 'text]) (mold/only record-value)
                    ]
                ] [
                    set (to-path reduce [to-word field-name 'text]) (to-string record-value)
                ]
            ]
        ] 
        requester-complete-field: to-word rejoin ["requester-completed-field" tilde identifier] 
    ...
    evo-target-object-name: copy "amazing" 
    ...
    refresh-results-gui: does [
        save rejoin [project-path %vid-code-test-field.red] vid-code-test/text 
