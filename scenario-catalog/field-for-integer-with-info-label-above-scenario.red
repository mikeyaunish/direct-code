Red [
	Title: "field-for-integer-with-info-label-above-scenario.red"
	Needs: View
...
field-for-integer-with-info-label-above-scenario-layout: [
	...
	style field-root: field extra [
	    setup-style: [ ;-- objects/3 - because of sequence in the resulting instances below
	        [
	            input [
	                prompt "Field name" 
	...
	style x-axis-button-left: x-axis-button-root 24x24
		...
		extra [
		    ...
		    directions: copy [] 
		    original-color: 230.230.230 
		    setup-style: [ ;-- objects/2 because of the sequence in the instances below
		        [
		            action [
		                alter-facet/value 'with compose/deep [
		                		extra/output-field: (to-lit-word objects/3/input-values/1)
		                	]
		            ]
		        ]
		    ]
    ...
    space 1x2
    label-left1: label-above "Shoe size"													;-- objects/1
	button-info1: button-info with [extra/box: 1 extra/message: "The shoe size to record"]  ;-- objects/2
    return 																					
    f1: field-root																			;-- objects/3
